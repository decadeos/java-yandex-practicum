...
public class DinnerConstructor {
    ...
    //метод для проверки дубликатов ТИПОВ* блюд
    ...
    // рассчитывает максимально возможное количество уникальных комбинаций блюд для заданного списка типов блюд
    public int getMaxCombinations(ArrayList<String> types) {
        int maxCombos = 1;
        for (String type : types) {
            if (!dinnersByType.containsKey(type)) return 0;
            maxCombos *= dinnersByType.get(type).size();
        }
        return maxCombos;
    }
